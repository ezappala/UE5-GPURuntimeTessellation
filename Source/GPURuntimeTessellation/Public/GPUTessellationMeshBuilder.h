 ...
 */
struct FGPUTessellationBuffers final : FRenderResource {
    ...
    class FGPUIndexBuffer final : public FIndexBuffer {
    public:
        ...
        virtual void ReleaseRHI() override { IndexBufferRHI.SafeRelease(); }
    };
 ...
 */
struct FGPUTessellationPatchBuffers final : FRenderResource {
    ...
    bool IsValid() const { return PatchBuffers.Num() > 0 && PatchBuffers.Num() == GetTotalPatchCount(); }

    void Reset() {
        for (FGPUTessellationBuffers& Patch : PatchBuffers) { Patch.Reset(); }
        PatchBuffers.Empty();
        PatchInfo.Empty();
        PatchCountX = 1;
        PatchCountY = 1;
...
private:
     ...
     */
    static FIntPoint CalculateResolution(float TessellationFactor);

    /**
     * Dispatch vertex generation compute shader
     */
    static void DispatchVertexGeneration(
        FRDGBuilder& GraphBuilder,
        const FGPUTessellationSettings& Settings,
     ...
     */
    static void DispatchIndexGeneration(
        FRDGBuilder& GraphBuilder,
        FIntPoint Resolution,
     ...
     */
    static FRDGTextureRef GetDefaultWhiteTexture(FRDGBuilder& GraphBuilder);
     ...
     */
    static void ComputePatchEdgeTransitions(
        int32 PatchCountX,
        int32 PatchCountY,
        TArray<FGPUTessellationPatchInfo>& PatchInfo
    );
     ...
     */
    static int32 ConvertPatchLevelToTessellation(EGPUTessellationPatchLevel Level);
};
