...
void FGPURuntimeTessellationModule::StartupModule() {
    ...
    const FString PluginShaderDir = FPaths::Combine(
        IPluginManager::Get().FindPlugin(TEXT("GPURuntimeTessellation"))->GetBaseDir(), TEXT("Shaders"));
    AddShaderSourceDirectoryMapping(TEXT("/Plugin/GPURuntimeTessellation"), PluginShaderDir);

    UE_LOG(LogTemp, Log, TEXT("GPURuntimeTessellation: Module started, shader directory mapped to: %s"),
        *PluginShaderDir);
}
