...
void FGPUTessellationMeshBuilder::ExecuteTessellationPipeline(
    ...
    const FIntPoint Resolution = CalculateResolution(Settings.TessellationFactor);
    ...
    DispatchVertexGeneration(GraphBuilder, Settings, Resolution, LocalToWorld, FVector::ZeroVector, VertexBuffer,
        NormalBuffer, UVBuffer);

    // Step 2: Apply displacement
    DispatchDisplacement(GraphBuilder, Settings, Resolution, DisplacementTexture, SubtractTexture, VertexBuffer,
        NormalBuffer, UVBuffer);

    // Step 3: Calculate normals (if enabled)
    if (Settings.NormalCalculationMethod != EGPUTessellationNormalMethod::Disabled) {
        DispatchNormalCalculation(GraphBuilder, Settings, Resolution, DisplacementTexture, SubtractTexture,
            NormalMapTexture, VertexBuffer, NormalBuffer, UVBuffer);
    }
...
void FGPUTessellationMeshBuilder::ExecuteTessellationPipeline(
    ...
    // Calculate resolution
    const FIntPoint Resolution = CalculateResolution(Settings.TessellationFactor);
    const int32 VertexCount = Resolution.X * Resolution.Y;
    const int32 IndexCount = (Resolution.X - 1) * (Resolution.Y - 1) * 6;
    ...
    GraphBuilder.AddPass(
        ...
        ERDGPassFlags::None,
        [&OutGPUBuffers, PooledPositionBuffer, PooledNormalBuffer, PooledUVBuffer, PooledIndexBuffer](
        FRHICommandList& RHICmdList
            ...
            if (PooledIndexBuffer.IsValid()) {
                ...
                if (!OutGPUBuffers.IndexBuffer.IsInitialized()) { OutGPUBuffers.IndexBuffer.InitResource(RHICmdList); }
            }
...
void FGPUTessellationMeshBuilder::GenerateMeshSync(
    ...
    ENQUEUE_RENDER_COMMAND(GenerateTessellatedMesh)(
        [this, Settings, LocalToWorld, CameraPosition, DisplacementTexture, RVTMaskTexture, &OutMeshData](
        FRHICommandListImmediate& RHICmdList
    ) {
            FRDGBuilder GraphBuilder(RHICmdList);

            ExecuteTessellationPipeline(GraphBuilder, Settings, LocalToWorld, CameraPosition, DisplacementTexture,
                RVTMaskTexture, nullptr, OutMeshData);
...
FIntPoint FGPUTessellationMeshBuilder::CalculateResolution(const float TessellationFactor) {
    ...
    constexpr int32 ThreadGroupSize = 8; // must match THREADGROUP_SIZE_X/Y in shaders
    constexpr int32 MaxResolution = 1024;
    constexpr int32 MaxSegments = MaxResolution - 1;
...
void FGPUTessellationMeshBuilder::DispatchVertexGeneration(
    ...
    FGPUVertexGenerationCS::FParameters* PassParameters = GraphBuilder.AllocParameters<
        FGPUVertexGenerationCS::FParameters>();
...
void FGPUTessellationMeshBuilder::DispatchDisplacement(
    ...
    UTexture* DisplacementTexture,
    UTexture* SubtractTexture,
    const FRDGBufferRef VertexBuffer,
    const FRDGBufferRef NormalBuffer,
    const FRDGBufferRef UVBuffer
) {
    const int32 VertexCount = Resolution.X * Resolution.Y;

    // Get or create textures
    const FRDGTextureRef DisplacementTextureRDG = DisplacementTexture
    ...
    const FRDGTextureRef SubtractTextureRDG = SubtractTexture
    ...
    PassParameters->DisplacementSampler = TStaticSamplerState<SF_Bilinear>::GetRHI();
    PassParameters->RVTMaskTexture = GraphBuilder.CreateSRV(FRDGTextureSRVDesc::Create(SubtractTextureRDG));
    PassParameters->RVTMaskSampler = TStaticSamplerState<SF_Bilinear>::GetRHI();
...
void FGPUTessellationMeshBuilder::DispatchNormalCalculation(
    ...
    const FRDGTextureRef NormalMapTextureRDG = NormalMapTexture
    ...
    // Setup shader parameters
    FGPUNormalCalculationCS::FParameters* PassParameters = GraphBuilder.AllocParameters<
        FGPUNormalCalculationCS::FParameters>();
    PassParameters->NormalCalculationMethod = static_cast<uint32>(Settings.NormalCalculationMethod);
    ...
    PassParameters->SubtractSampler = TStaticSamplerState<SF_Bilinear>::GetRHI();
    ...
    PassParameters->NormalMapSampler = TStaticSamplerState<SF_Bilinear>::GetRHI();
    PassParameters->InputPositions = GraphBuilder.CreateSRV(VertexBuffer);
    PassParameters->InputUVs = GraphBuilder.CreateSRV(UVBuffer);
    PassParameters->InputIndices = GraphBuilder.CreateSRV(DummyIndexBuffer);
...
void FGPUTessellationMeshBuilder::DispatchIndexGeneration(
    ...
    // Setup shader parameters
    FGPUIndexGenerationCS::FParameters* PassParameters = GraphBuilder.AllocParameters<
...
void FGPUTessellationMeshBuilder::ExtractMeshData(
    ...
    const FRDGBufferRef NormalBuffer,
    const FRDGBufferRef UVBuffer,
    const FRDGBufferRef IndexBuffer,
    FGPUTessellatedMeshData& OutMeshData
) {
    int32 VertexCount = Resolution.X * Resolution.Y;
    int32 IndexCount = (Resolution.X - 1) * (Resolution.Y - 1) * 6;
    ...
    GraphBuilder.AddPass(
        ...
        ERDGPassFlags::None,
        [VertexReadback, NormalReadback, UVReadback, IndexReadback, &OutMeshData, VertexCount, IndexCount](
        FRHICommandListImmediate& RHICmdList
    ) {
...
FRDGTextureRef FGPUTessellationMeshBuilder::CreateRDGTextureFromUTexture(
    ...
    if (!Texture || !Texture->GetResource()) { return GetDefaultWhiteTexture(GraphBuilder); }
...
void FGPUTessellationMeshBuilder::ExecutePatchTessellationPipeline(
    ...
    const int32 PatchCountX,
    const int32 PatchCountY,
    UTexture* DisplacementTexture,
    UTexture* SubtractTexture,
    UTexture* NormalMapTexture,
    ...
    const FVector Location = LocalToWorld.GetOrigin();
    const FVector Scale = LocalToWorld.GetScaleVector();
    ...
    const int32 TotalPatches = PatchCountX * PatchCountY;
    ...
    for (int32 PatchIndex = 0; PatchIndex < TotalPatches; ++PatchIndex) {
        ...
        if (PatchIndex < 4) {
            UE_LOG(LogTemp, Warning,
                TEXT("  Patch[%d]: UV:(%.3f,%.3f) Size:(%.3f,%.3f) LOD:%d Visible:%d WorldCenter:%s"),
                PatchIndex,
    ...
    // Log summary
    UE_LOG(LogTemp, Warning,
        TEXT("GPUTessellation: Patch Generation Summary - Total:%d Generated:%d SkippedCulled:%d SkippedInvalidLOD:%d"),
...
void FGPUTessellationMeshBuilder::GenerateSinglePatch(
    ...
    if (TessellationLevel <= 0) {
        UE_LOG(LogTemp, Error,
            TEXT("GPUTessellation: GenerateSinglePatch - Invalid TessellationLevel=%d (must be > 0)"),
            TessellationLevel);
    ...
    if (Resolution.X < 2 || Resolution.Y < 2) {
        UE_LOG(LogTemp, Error,
            TEXT("GPUTessellation: GenerateSinglePatch - Invalid resolution %dx%d (must be at least 2x2)"),
    ...
    if (PatchInfo.PatchOffset.X < 0.0f || PatchInfo.PatchOffset.Y < 0.0f ||
        ...
        UE_LOG(LogTemp, Error,
            TEXT("GPUTessellation: GenerateSinglePatch - Invalid UV parameters: Offset=(%.3f,%.3f) Size=(%.3f,%.3f)"),
    ...
    DispatchVertexGeneration(GraphBuilder, PatchSettings, Resolution, LocalToWorld, FVector::ZeroVector, VertexBuffer,
        NormalBuffer, UVBuffer);

    // Step 2: Apply displacement (samples texture at patch's UV range using same UV offset/scale)
    DispatchDisplacement(GraphBuilder, PatchSettings, Resolution, DisplacementTexture, SubtractTexture, VertexBuffer,
        NormalBuffer, UVBuffer);

    // Step 3: Calculate normals if enabled (also use patch settings for consistent plane size)
    if (Settings.NormalCalculationMethod != EGPUTessellationNormalMethod::Disabled) {
        DispatchNormalCalculation(GraphBuilder, PatchSettings, Resolution, DisplacementTexture, SubtractTexture,
            NormalMapTexture, VertexBuffer, NormalBuffer, UVBuffer);
    }
    ...
    GraphBuilder.AddPass(
        ...
        ERDGPassFlags::None,
        [&OutPatchBuffers, PooledPositionBuffer, PooledNormalBuffer, PooledUVBuffer, PooledIndexBuffer](
        FRHICommandList& RHICmdList
    ) {
            ...
            // Initialize the index buffer as a render resource if not already initialized
            if (!OutPatchBuffers.IndexBuffer.IsInitialized()) { OutPatchBuffers.IndexBuffer.InitResource(RHICmdList); }
        }
    );
}
...
void FGPUTessellationMeshBuilder::ComputePatchEdgeTransitions(
    ...
    if (PatchCountX <= 0 || PatchCountY <= 0 || PatchInfo.Num() != ExpectedCount) { return; }

    const auto GetPatch = [&](const int32 X, const int32 Y) -> const FGPUTessellationPatchInfo* {
        if (X < 0 || X >= PatchCountX || Y < 0 || Y >= PatchCountY) { return nullptr; }
    ...
    for (int32 Y = 0; Y < PatchCountY; ++Y) {
        for (int32 X = 0; X < PatchCountX; ++X) {
            ...
            auto ComputeFactor = [&](const FGPUTessellationPatchInfo* Neighbor, const bool bVerticalEdge) -> int32 {
                if (!Neighbor) { return 1; }
                if (Patch.ResolutionX <= 0 || Patch.ResolutionY <= 0 || Neighbor->ResolutionX <= 0 || Neighbor->
                    ResolutionY <= 0) { return 1; }
                if (Neighbor->TessellationLevel >= Patch.TessellationLevel) { return 1; }

                const int32 MySegments = bVerticalEdge
                    ? FMath::Max(1, Patch.ResolutionY - 1)
                    : FMath::Max(1, Patch.ResolutionX - 1);
                const int32 NeighborSegments = bVerticalEdge
                    ? FMath::Max(1, Neighbor->ResolutionY - 1)
                    : FMath::Max(1, Neighbor->ResolutionX - 1);
                if (NeighborSegments <= 0 || MySegments <= NeighborSegments) { return 1; }
...
void FGPUTessellationMeshBuilder::CalculatePatchInfo(
    ...
    const int32 PatchCountX,
    const int32 PatchCountY,
    TArray<FGPUTessellationPatchInfo>& OutPatchInfo
) const {
    const int32 TotalPatches = PatchCountX * PatchCountY;
    ...
    const float PatchUVSizeX = 1.0f / static_cast<float>(PatchCountX);
    const float PatchUVSizeY = 1.0f / static_cast<float>(PatchCountY);

    // Each patch's size in LOCAL space (renamed from "PatchWorldSize" for clarity)
    const float PatchLocalSizeX = PlaneSizeX / static_cast<float>(PatchCountX);
    const float PatchLocalSizeY = PlaneSizeY / static_cast<float>(PatchCountY);

    FVector PlaneOrigin = LocalToWorld.GetOrigin();

    for (int32 Y = 0; Y < PatchCountY; ++Y) {
        for (int32 X = 0; X < PatchCountX; ++X) {
            const int32 PatchIndex = Y * PatchCountX + X;
            ...
            const float LocalMinX = (Patch.PatchOffset.X - 0.5f) * PlaneSizeX;
            const float LocalMinY = (Patch.PatchOffset.Y - 0.5f) * PlaneSizeY;
            ...
            const float LocalCenterZ = Settings.DisplacementOffset + Settings.DisplacementIntensity * 0.5f;
            ...
            if (PatchIndex < 4) {
                UE_LOG(LogTemp, Warning,
                    TEXT("  CalcPatchInfo[%d]: LocalMin=(%.1f, %.1f) LocalCenter=(%.1f, %.1f) WorldCenter=%s"),
            ...
            const float MaxDisplacementUp = Settings.DisplacementIntensity + FMath::Max(0.0f,
                Settings.DisplacementOffset);
            const float MaxDisplacementDown = FMath::Abs(FMath::Min(0.0f, Settings.DisplacementOffset));
            const float TotalDisplacementRange = MaxDisplacementUp + MaxDisplacementDown;

            const FVector HalfExtent(PatchLocalSizeX * 0.5f, PatchLocalSizeY * 0.5f, TotalDisplacementRange * 0.5f);

            // Build bounds by transforming corners
            // The bounds extend from the center position in all directions by HalfExtent
            TArray<FVector> Corners;
            Corners.Add(
                LocalToWorld.TransformPosition(LocalCenter + FVector(-HalfExtent.X, -HalfExtent.Y, -HalfExtent.Z)));
            Corners.Add(
                LocalToWorld.TransformPosition(LocalCenter + FVector(-HalfExtent.X, -HalfExtent.Y, +HalfExtent.Z)));
            Corners.Add(
                LocalToWorld.TransformPosition(LocalCenter + FVector(-HalfExtent.X, +HalfExtent.Y, -HalfExtent.Z)));
            ...
            const FIntPoint PatchResolution = CalculateResolution(static_cast<float>(Patch.TessellationLevel));
            ...
            if (PatchIndex < 8 || (PatchIndex == 0 && Patch.TessellationLevel <= 0)) {
                UE_LOG(LogTemp, Warning,
                    TEXT("    Patch[%d]: PatchCenter=%s CameraPos=%s Distance=%.1f -> LOD:%d (Tess=%d)"),
                    PatchIndex,
            ...
            if (Patch.TessellationLevel <= 0) {
                UE_LOG(LogTemp, Error,
                    TEXT("    Patch[%d]: INVALID TessellationLevel=%d! Distance=%.1f CameraPos=%s PatchCenter=%s"),
...
int32 FGPUTessellationMeshBuilder::CalculatePatchTessellationLevel(
    ...
    if (Settings.PatchDistances.Num() == 0) { return ConvertPatchLevelToTessellation(Settings.PatchLevels[0]); }
    ...
    for (int32 i = 0; i < Settings.PatchDistances.Num(); ++i) {
        if (DistanceToCamera <= Settings.PatchDistances[i]) {
            ...
            if (i < Settings.PatchLevels.Num()) { TargetLevel = static_cast<int32>(Settings.PatchLevels[i]); }
            break;
        }
    }
    ...
    if (DistanceToCamera > Settings.PatchDistances[Settings.PatchDistances.Num() - 1]) {
        ...
        const int32 LastIndex = FMath::Min(Settings.PatchDistances.Num(), Settings.PatchLevels.Num()) - 1;
...
int32 FGPUTessellationMeshBuilder::ConvertPatchLevelToTessellation(const EGPUTessellationPatchLevel Level) {
